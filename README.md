...
<script>
let map, pathPolyline, startMarker=null, endMarker=null;
let watchId=null, pathCoords=[], distanceKm=0, startTime=0, elapsedTime=0, timerInterval=null;

const baseFare = 60;
const kmFare = 15;
const timeFare = 3; // 每分鐘
...
function startTrip(){
    ...
    if(!watchId && navigator.geolocation){
        watchId = navigator.geolocation.watchPosition(pos=>{
            ...
            if(!startMarker) startMarker = new google.maps.Marker({position:newCoord,map:map,icon:"https://maps.google.com/mapfiles/ms/icons/green-dot.png"});
...
function endTrip(){
    pauseTrip();
    if(pathCoords.length>0 && !endMarker){
        const last = pathCoords[pathCoords.length-1];
        endMarker = new google.maps.Marker({position:last,map:map,icon:"https://maps.google.com/mapfiles/ms/icons/red-dot.png"});
    }
